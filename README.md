...
- 20200311 V1.2
    - 1.根据情况对服务器做了分组调整
    - 2.仓库配置文件中可以对覆盖全局配置，从而将仓库发布在指定的机器上
    - 3.由于php环境变量的使用，不再关心php路径和版本
- 20190812 V1.1.3
    - 1.为了符合git相关规范调整了操作时注释的内容
- 20190808 V1.1.2
    - 1.为了符合git相关规范调整了操作时注释的内容
- 20190730 V1.1.1
    - 1.为了符合git相关规范调整了操作时注释的内容
    - 2.关闭debug模式
- 20190705 V1.1
    - 1.增加自定义创建上传代码文件的步骤,针对仓库代码需要处理后,再上传到服务器的情况
- 20190625 V1.0.1
    - 1.增加lbsapi
- 20190530 V1.0.1
    - 1.小的调整
    - 2.以后增加仓库和仓库本身的调整,都不再更新版本号
    - 3.下个版本增加每个仓库在迁出代码后可以执行自定义的操作
- 20190527 V1.0
